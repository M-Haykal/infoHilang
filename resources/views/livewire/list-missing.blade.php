<div class="py-20 max-w-7xl mx-auto px-4">
    ...
    <div class="text-center mb-10">
        <h2 class="text-3xl md:text-4xl font-extrabold text-slate-800">Daftar Hilang <span class="text-blue-600">&amp;</span> Ditemukan</h2>
    ...
    <div class="sticky top-[72px] z-40 bg-slate-50 backdrop-blur-md py-4 transition-all duration-300">
        <div class="max-w-7xl mx-auto">
            <label class="flex flex-col min-w-40 h-14 w-full">
                <div class="flex w-full flex-1 items-stretch rounded-2xl h-full shadow-lg bg-white border border-slate-200">
                    <div class="text-gray-500 flex items-center justify-center pl-5">
                        <i class="fa-solid fa-magnifying-glass text-xl"></i>
                    </div>
                    <input wire:model.live.debounce.500ms="search" wire:key="search-input-{{ $search === '' ? 'empty' : 'active' }}" class="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-2xl text-slate-800 border-none bg-transparent h-full px-4 pl-3 text-base font-medium placeholder:text-gray-500 focus:outline-none focus:ring-0 focus:border-none focus:shadow-none" placeholder="Cari barang, hewan, atau orang..." type="text" />
                </div>
            </label>
        </div>
    </div>
            ...
            <div class="sticky top-[160px] z-30 bg-white rounded-xl shadow-md p-5 border border-gray-200">
                ...
                <div class="grid grid-cols-1 lg:flex lg:flex-col gap-6">
                    ...
                    <div class="col-span-1" wire:key="filter-status-container">
                        ...
                        <div class="lg:hidden">
                            <select wire:model.live="status" wire:key="select-status-{{ $status === '' ? 'reset' : 'active' }}" class="w-full rounded-lg border border-gray-200 text-sm font-medium p-3 transition-all text-gray-500 focus:border-primary focus:ring-primary focus:outline-none">
                        ...
                        <div class="hidden lg:flex flex-col gap-2">
                            ...
                            <label wire:key="wrapper-status-{{ $val }}" class="flex items-center gap-3 rounded-lg border p-3 cursor-pointer transition-colors {{ $status === $val ? 'border-primary bg-primary/5' : 'border-gray-200 hover:border-primary/50' }}">
                                ...
                                <p class="text-sm font-medium {{ $status === $val ? 'text-primary' : 'text-gray-500' }}">
                                    {{ $label }}
                                </p>
                            </label>
                            @endforeach
                    ...
                    <div class="col-span-1" wire:key="filter-kategori-container">
                        ...
                        <div class="lg:hidden">
                            <select wire:model.live="kategori" wire:key="select-kategori-{{ $kategori === '' ? 'reset' : 'active' }}" class="w-full rounded-lg border border-gray-200 text-sm font-medium p-3 transition-all text-gray-500 focus:border-primary focus:ring-primary focus:outline-none">
                        ...
                        <div class="hidden lg:flex flex-col gap-2">
                            ...
                            <label wire:key="wrapper-kategori-{{ $val }}" class="flex items-center gap-3 rounded-lg border p-3 cursor-pointer transition-colors {{ $kategori === $val ? 'border-primary bg-primary/5 shadow-sm' : 'border-gray-200 hover:border-primary/50' }}">
                                ...
                                <p class="text-sm font-medium {{ $kategori === $val ? 'text-primary' : 'text-gray-500' }}">
                                    {{ $label }}
                                </p>
                            </label>
                            @endforeach
                    ...
                    <div class="col-span-1">
                        ...
                        <div class="relative">
                            ...
                            <input wire:model.live.debounce.500ms="lokasi" wire:key="filter-lokasi-{{ $lokasi ? 'active' : 'empty' }}" class="form-input w-full rounded-lg border-gray-300 text-sm font-medium text-gray-500 focus:border-primary focus:ring-primary pl-10 py-2 bg-slate-100" placeholder="Jakarta Selatan" type="text" />
                        </div>
                    </div>
                    ...
                    <div class="col-span-1">
                        ...
                        <div class="relative">
                            ...
                            <input id="filter-date" wire:model.live="date" wire:key="filter-date-{{ $date ? 'active' : 'empty' }}" class="form-input w-full rounded-lg border-gray-300 text-sm font-medium text-gray-500 focus:border-primary focus:ring-primary pl-10 py-2 bg-slate-100" type="date" onclick="this.showPicker()" />
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="w-full lg:w-3/4 xl:w-4/5 flex flex-col">
            <div class="flex items-center justify-end py-2 sticky top-[152px] z-30 pt-3">
                <div class="flex items-center gap-1 bg-slate-100 p-1 rounded-xl">
                    <button wire:click="setView('grid')" class="p-2 rounded-lg transition-all {{ $viewMode === 'grid' ? 'bg-white shadow-sm text-orange-600' : 'text-slate-400 hover:text-gray-500' }}">
                        <i class="fa-solid fa-grip text-lg"></i>
                    </button>
                    <button wire:click="setView('list')" class="p-2 rounded-lg transition-all {{ $viewMode === 'list' ? 'bg-white shadow-sm text-orange-600' : 'text-slate-400 hover:text-gray-500' }}">
            ...
            <div class="flex-1 flex flex-col mt-4">
                ...
                <div class="{{ $viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-3 gap-6' : 'flex flex-col gap-4' }}">
                    ...
                    <div class="group flex {{ $viewMode === 'grid' ? 'flex-col' : 'flex-row' }} overflow-hidden rounded-2xl border bg-white shadow-sm hover:shadow-md transition-all duration-300">
                        ...
                        <div class="relative overflow-hidden {{ $viewMode === 'grid' ? 'w-full aspect-square' : 'w-[36%] md:w-48 flex-shrink-0' }}">
                            ...
                            <span class="absolute top-3 left-3 {{ $report->status == 'Hilang' ? 'bg-red-600' : 'bg-green-600' }} text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                        ...
                        <div class="{{ $viewMode === 'grid' ? 'w-full' : 'w-2/3 md:w-48' }} flex flex-1 flex-col p-5 justify-between">
                            <div>
                                <div class="flex justify-between items-center mb-2">
                                    ...
                                    @if($viewMode === 'list')
                                    <span class="hidden md:block text-[10px] text-slate-400 font-medium tracking-widest">{{ $report->created_at->diffForHumans() }}</span>
                                    @endif
                                </div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="flex items-center text-xs text-gray-500 bg-slate-50 p-2 rounded-lg border border-slate-100">
                            ...
                            <div class="mt-4 flex {{ $viewMode === 'grid' ? '' : 'justify-end' }}">
                                <a href="" class="flex items-center justify-center bg-slate-800 hover:bg-slate-900 text-white font-bold text-sm py-2 rounded-lg transition {{ $viewMode === 'grid' ? 'w-full' : 'w-fit px-4' }}">Detail Laporan</a>
                            </div>
                        </div>
                    </div>
                    @endforeach
                ...
                @else
                <div class="flex-1 flex flex-col items-center justify-center bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200 p-10 text-center min-h-[460px]">
                    ...
                    <p class="text-slate-400 mb-6">Coba ubah kata kunci atau gunakan tombol reset untuk mencari ulang.</p>
