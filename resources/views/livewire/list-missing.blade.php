    ...
    <div class="sticky top-[72px] z-40 bg-slate-50 backdrop-blur-md py-4 transition-all duration-300">
        <div class="max-w-7xl mx-auto">
            <label class="flex flex-col min-w-40 h-14 w-full">
                <div class="flex w-full flex-1 items-stretch rounded-2xl h-full shadow-lg bg-white dark:bg-secondary border border-slate-200">
                    <div class="text-slate-400 flex items-center justify-center pl-5">
                        <i class="fa-solid fa-magnifying-glass text-xl"></i>
                    </div>
                    <input wire:model.live.debounce.500ms="search" class="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-2xl text-slate-800
           border-none bg-transparent h-full px-4 pl-3 text-base font-medium
           placeholder:text-slate-400
           /* Hilangkan Semua Outline/Border/Ring saat Active */
           focus:outline-none focus:ring-0 focus:border-none focus:shadow-none" placeholder="Cari barang, hewan, atau orang..." type="text" />
                </div>
            </label>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-8 lg:flex-row">
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <div>
                        ...
                        <div class="flex flex-col gap-2">
                            <label class="flex items-center gap-3 rounded-lg border border-gray-200 dark:border-gray-600 p-3 cursor-pointer hover:border-primary/50 transition-colors">
                                <input wire:model.live="status" value="Hilang" name="status" class="h-4 w-4 text-primary focus:ring-primary" type="radio" />
                                <p class="text-sm font-medium text-gray-700 dark:text-gray-600">Hilang</p>
                            </label>
                            <label class="flex items-center gap-3 rounded-lg border border-gray-200 dark:border-gray-600 p-3 cursor-pointer hover:border-primary/50 transition-colors">
                                <input wire:model.live="status" value="Ditemukan" name="status" class="h-4 w-4 text-primary focus:ring-primary" type="radio" />
                    ...
                    <div>
                        ...
                        <div class="flex flex-col gap-2">
                            <label class="flex items-center gap-3 rounded-lg border border-gray-200 dark:border-gray-600 p-3 cursor-pointer hover:border-primary/50 transition-colors">
                                <input wire:model.live="kategori" value="Barang" name="kategori" class="h-4 w-4 text-primary focus:ring-primary" type="radio" />
                                <p class="text-sm font-medium text-gray-700 dark:text-gray-600">Barang</p>
                            </label>
                            <label class="flex items-center gap-3 rounded-lg border border-gray-200 dark:border-gray-600 p-3 cursor-pointer hover:border-primary/50 transition-colors">
                                <input wire:model.live="kategori" value="Hewan" name="kategori" class="h-4 w-4 text-primary focus:ring-primary" type="radio" />
                                <p class="text-sm font-medium text-gray-700 dark:text-gray-600">Hewan</p>
                            </label>
                            <label class="flex items-center gap-3 rounded-lg border border-gray-200 dark:border-gray-600 p-3 cursor-pointer hover:border-primary/50 transition-colors">
                                <input wire:model.live="kategori" value="Orang" name="kategori" class="h-4 w-4 text-primary focus:ring-primary" type="radio" />
                                <p class="text-sm font-medium text-gray-700 dark:text-gray-600">Orang</p>
                            </label>
                            <label class="flex items-center gap-3 rounded-lg border border-gray-200 dark:border-gray-600 p-3 cursor-pointer hover:border-primary/50 transition-colors">
                                <input wire:model.live="kategori" value="" name="kategori" class="h-4 w-4 text-primary focus:ring-primary" type="radio" />
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input wire:model.live.debounce.500ms="lokasi" class="form-input w-full rounded-lg border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-100 focus:border-primary focus:ring-primary/50 pl-10 py-2 text-sm" placeholder="e.g. Jakarta Selatan" type="text" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input wire:model.live="date" class="form-input w-full rounded-lg border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-100 focus:border-primary focus:ring-primary/50 pl-10 py-2 text-sm" type="date" />
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="w-full lg:w-3/4 xl:w-4/5">
            ...
            <div class="flex items-center justify-end py-2 sticky top-[152px] z-30 pt-3">
                <div class="flex items-center gap-1 bg-slate-100 p-1 rounded-xl">
                    <button wire:click="setView('grid')" class="p-2 rounded-lg transition-all {{ $viewMode === 'grid' ? 'bg-white shadow-sm text-orange-600' : 'text-slate-400 hover:text-slate-600' }}">
                        <i class="fa-solid fa-grip text-lg"></i>
                    </button>
                    <button wire:click="setView('list')" class="p-2 rounded-lg transition-all {{ $viewMode === 'list' ? 'bg-white shadow-sm text-orange-600' : 'text-slate-400 hover:text-slate-600' }}">
                        <i class="fa-solid fa-list text-lg"></i>
                    </button>
                </div>
            </div>

            <div class="py-4 {{ $viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-3 xl:grid-cols-3 gap-6' : 'flex flex-col gap-4' }}">
                    ...
                    <div class="{{ $viewMode === 'grid' ? 'w-full' : 'w-2/3 md:w-48' }} flex flex-1 flex-col p-5 justify-between">
                        <div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center text-xs text-slate-600 bg-slate-50 p-2 rounded-lg border border-slate-100">
